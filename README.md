...
| E3 | 선형 연립방정식 | 대각이 `δ = 10⁻ⁿ`(`n = 1–16`)인 4×4 행렬, 풀이법 5종, 단순 가우스 소거·partial pivoting의 소수점 아래 `k`자리 반올림(`n` = 1, 2, 4, 8) |
| E4 | 분포 회귀 | 정규분포 표본 100개의 경험적 CDF를 모델 5종으로 적합, seed 100개 |

- **오차:** 참 백분율 상대오차 `ε_t = |(참값 − 근삿값) / 참값| · 100`(%), 근사 백분율
  상대오차 `ε_a = |(현재 − 이전) / 현재| · 100`(%).
- **표기:** 측정값은 유효숫자 4자리로 표기한다. `0`은 float64에서 정확히 0, `nan`은 계산
  결과가 NaN임을 뜻한다.
...
`n` 16개 중 6개와 역행렬 풀이를 뺀 풀이법만 표시하며, 전체 결과는 로그 E3-b에 있다.*
