...
**기간:** E1–E2 2024.03.25 – 2024.04.14, E3–E4 2024.05.02 – 2024.05.26

## 개요

Taylor 급수, 근 찾기, 선형 연립방정식, 회귀의 고전 수치해석 기법을 SciPy 없이
NumPy로 직접 구현하고, 각 기법의 오차를 측정한다. 비교 기준인 역행렬 풀이와
정규방정식 풀이, 조건수·고유값 계산에는 `numpy.linalg`를 사용한다. 네 실험은 서로 독립적이다. 모든
...
| E3 | 선형 연립방정식 | 대각이 `δ = 10⁻ⁿ`(`n = 1–16`)인 4×4 행렬, 풀이법 5종과 `k`자리 반올림 |
| E4 | 분포 회귀 | 정규분포 표본 100개의 경험적 CDF를 모델 5종으로 적합, seed 100개 |

- **오차:** 참 상대오차 `ε_t = |(참값 − 근삿값) / 참값| · 100`(%), 근사오차
  `ε_a = |(현재 − 이전) / 현재| · 100`(%).
...
*그림 2. iteration별 참 상대오차(log scale). 오차가 0인 기록은 그리지 않았다.*

### E3: 선형 연립방정식

*참 상대오차 `ε_t`(%)의 네 성분 중 최댓값. 기준은 유리수 연산으로 구한 정확해이다.*
...
*그림 3. `δ`에 따른 오차(좌)와 `k`자리 반올림에 따른 오차(우).*
